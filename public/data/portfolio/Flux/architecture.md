...
- **Key responsibilities**: Manages 34 sound layers across 4 categories (nature, noise, tonal, ambient) — 24 backed by MP3 files and 10 web-synthesized — provides unified play/stop/volume API, handles Web Audio API synthesis (noise, oscillators, FM, binaural beats) on web, handles expo-audio file playback on native, runs real-time amplitude analysis (FFT on web, simulated on native), manages custom sounds and soundscapes, supports 3-band EQ and spatial audio (web only). Pure, side-effect-free logic (gain math, band normalization, soundscape layer resolution) is extracted into `lib/audioEngine.ts` so it can be unit-tested independently of the React tree.
...
- **Rationale**: SVG renders consistently across all three platforms without GL context issues. Reanimated provides 60fps animations on the UI thread. The earlier WebGL prototype dependencies (`three`, `@react-three/fiber`, `@react-three/drei`, `expo-gl`) have since been removed from `package.json`, shrinking the dependency surface. The SVG approach keeps the bundle smaller and avoids WebGL compatibility issues on older devices.
...
### Fibonacci Sphere Distribution from a Single Source
- **Context**: Particles need to be distributed evenly on a sphere surface, and three different consumers — the live render, the saved-creation thumbnail generator, and the gallery card preview — must produce visually identical layouts
- **Decision**: Golden angle distribution (`π × (3 - √5)`) lives in one shared module, `lib/fibonacciSphere.ts`, imported by `components/ParticleSphere.tsx`, `lib/thumbnailGenerator.ts`, and `lib/particlePreview.ts`
- **Rationale**: Fibonacci distribution produces the most visually even point distribution on a sphere (no clustering at poles like latitude/longitude grids). Earlier the algorithm was copy-pasted across all three call sites, which risked drift between what a user saw live and what their saved thumbnail showed. Centralizing it guarantees consistency and let the math be unit-tested in isolation (including edge cases like `numPoints = 1`, where the denominator is clamped to avoid divide-by-zero).
...
### Pure-Function Core with React as a Thin Shell
- **Context**: The audio and visualization logic — gain mixing, amplitude-band normalization, soundscape layer resolution, custom-sound persistence, preview playback state, reactive-background color derivation — was originally tangled inside React components and context providers, making it untestable without rendering and mocking the whole tree
- **Decision**: Extract the decision logic into a set of dependency-free modules (`lib/audioEngine.ts`, `lib/customSoundsStore.ts`, `lib/soundscapeColors.ts`, `lib/soundscapeSelection.ts`, `lib/previewPlayback.ts`, `lib/fibonacciSphere.ts`) and have the React layer call into them
- **Rationale**: Pure functions take inputs and return outputs with no hooks, no platform APIs, and no side effects, so they can be exercised directly by Jest without a renderer. This moved the bulk of the app's correctness-critical logic — divide-by-zero guards on master volume, FFT band splitting, soundscape gain tables, custom-sound storage rules — into a fast, deterministic test suite (`__tests__/lib/`), while the components stayed small and focused on rendering.
