...
|----------|------------|---------|-----------------|
| Interface | Claude Code skills | n/a | The primary front end: `prepare-1120` and `prepare-schedule-c` orchestrate the engine conversationally, gating categorization on user confirmation. Shipping the workflow as a skill keeps the model as an orchestrator and pushes every dollar of math down into deterministic Python. |
...
| Headless AI path | anthropic | 0.40+ | Powers the raw CLI when no Claude Code session is driving it: unknown bank-statement formats and unmatched-merchant categorization, gated on `ANTHROPIC_API_KEY`. Redundant under the skill (the in-session model does this work directly), and the CLI is fully usable without it on known layouts. |
...
## Interface (skill layer)

- **Skills**: `.claude/skills/prepare-1120/SKILL.md` and `.claude/skills/prepare-schedule-c/SKILL.md`, the primary way the toolkit is used
- **Role**: orchestration only. The skill runs the engine's CLI subcommands at the right time, reads the unmatched-transactions JSON the engine emits, proposes categories, and gates every write-back on user confirmation. No tax-law knowledge lives in the skill.
- **Why a skill drives it**: keeps the model out of the arithmetic. It decides *when* to run a step and explains the result; the Python engine decides *what the numbers are*. That division is what makes the output auditable.
...
- **Entry point**: `tax-toolkit = "tax_toolkit.cli:app"` (declared in `pyproject.toml`), the CLI seam the skill drives and the headless path uses directly
...
| `anthropic` | Headless path only (redundant when the skill drives the engine). Two methods on the SDK wrapper: `extract_transactions_from_text` (sends pdfplumber-extracted text) and `extract_transactions_from_pdf` (multimodal, sends the PDF as a base64-encoded document attachment). Both return strict JSON the engine deserializes into `Transaction` objects. |
