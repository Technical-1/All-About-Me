...
The original QuickPass v1 was a pure Rust desktop app using eframe/egui. While this kept everything in one language, the UI was limited. I migrated the entire application to Tauri v2 with a React frontend while keeping all cryptographic operations in Rust. The key challenge was designing a clean IPC boundary — 55 commands across 6 modules — that never leaks sensitive data to the web layer. Passwords cross the boundary only for clipboard copy, and the frontend only receives masked DTOs (`"********"`).
...
Passwords lingering in memory are attack vectors. I used Rust's `zeroize` crate with `Drop` implementations to ensure all sensitive data (vault key, decrypted entries, password hashes) is securely cleared when the session ends. The `subtle` crate provides constant-time comparison for TOTP verification and hash checking, preventing timing side-channel attacks. On top of this, IPC credential parameters use a dedicated `Credential` wrapper in `src-tauri/src/credential.rs` — `#[serde(transparent)]` for the wire format, no `Clone`, no `into_inner`, and a `Drop` impl that wipes the inner buffer regardless of how the function exits. This closes the panic-leak gap left by manual `mut password + .zeroize()` patterns where an early return skips the wipe.
...
220+ backend tests across three categories: ~160 unit tests covering encryption round-trips, CSV parsing, validation, credential lifecycle, clipboard supersession, and lockout state machines; ~20 command tests verifying the Tauri IPC layer including end-to-end backup round-trips; and ~40 integration tests for full workflows including import/export, security features, and cross-module verification.
...
Password and pattern attempts share a single budget: after the configured threshold (3-10 failures, default 5) the vault locks with exponential backoff (15min → 30min → 60min), then deletes after the 4th lockout. Touch ID has its own 5-attempt budget on a 5-minute cooldown — exhausting it disables Touch ID until a knowledge-based unlock, but doesn't lock the vault itself. TOTP failures track separately on the same 5-attempt / 5-minute pattern. All lockout state persists across restarts with an HMAC integrity check, and a vault-file backup of the lockout state prevents an attacker who deletes the lockout file from getting a fresh attempt budget.

### Why separate lockout budgets per auth path?
A single shared budget creates a budget-swap attack: exhaust the cheap path, fall back to the other with fresh attempts. Separate budgets let the rates reflect the actual cost (5/5min for Touch ID and TOTP, exponential cascade for password) without one path's failures consuming another's. Crucially the *reset* is asymmetric — a successful password proves knowledge and clears every lockout, but a successful Touch ID only clears its own. Knowledge supersedes possession; possession can't refresh knowledge attempts.
...
The codebase is open source with clear module separation. Critical security code is isolated in `src-tauri/src/vault.rs` (AES-256-GCM + dual-key encryption), `src-tauri/src/security.rs` (Argon2id parameters), `src-tauri/src/credential.rs` (IPC-boundary credential wrapper), and `src-tauri/src/clipboard.rs` (auto-clear lifecycle), so a reviewer can audit the primitives without reading the whole codebase. The 220+ backend tests include round-trip encryption checks, timing-safe comparison verification, lockout state machine coverage, and cross-module security regressions, so any regression on a hardened path fails CI before release.

### Can I check a released QuickPass binary against new CVEs?
Yes. Release binaries embed their `Cargo.lock` dependency manifest via `cargo-auditable`, so once a RUSTSEC advisory drops you can match it against a specific shipped artifact directly: `cargo audit bin /Applications/QuickPass.app/Contents/MacOS/quickpass`. No need to git-checkout the release tag to find out which dep versions were in that build. The README's "Auditing Released Builds" section has the full recipe.
